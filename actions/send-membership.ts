...
export async function sendMembershipEmail(formData: {
  firstName: string;
  lastName: string;
  phoneNumber: string;
  email: string;
  ...
  try {
          ...
          <div class="container">
            ...
            <div class="content">
              <div style="text-align: center; margin-bottom: 25px;">
                <span class="badge">NEW MEMBERSHIP APPLICATION</span>
              </div>
              ...
              <p style="color: #555; font-size: 15px; margin-bottom: 25px;">
                A new member has registered to join our community. Please find their registration details below:
              </p>
              ...
              <div class="form-section">
                ...
                <div class="form-row">
                  ...
                  <span class="value"><a href="tel:${formData.phoneNumber}">${formData.phoneNumber}</a></span>
                </div>

                <div class="form-row">
                  <span class="label">Email Address</span>
                  <span class="value">${formData.email ? `<a href="mailto:${formData.email}">${formData.email}</a>` : '<em style="color: #999;">Not provided</em>'}</span>
                </div>

                <div class="form-row">
                  <span class="label">About Them</span>
                  <div class="description-value">${formData.description}</div>
                </div>
              </div>

              <p style="color: #666; font-size: 14px; margin-top: 25px; font-style: italic;">
                Please reach out to this member soon to welcome them and provide information about our membership benefits and upcoming activities.
              </p>
            </div>
    ...
    const { data, error } = await resend.emails.send({
      ...
      subject: `New Member Registration: ${formData.firstName} ${formData.lastName}`,
      html: emailTemplate,
    });
